<table class="table table-striped table-bordered">
    ...
    <?php   if ($same_date == 'yes' && $successful_cancel_counts < 1 && $successful_refund_counts == 0 && $display_cancel_option == 'yes') { ?>
    ...
    <?php   } ?>
    <?php } ?>
    <?php if ( (is_array($iyzico_transactions) && count($iyzico_transactions) > 0) ||(is_string($iyzico_transactions) && $iyzico_transactions != 'false')) { ?>
    <tr>
        ...
        <td>
            <table class="table table-striped table-bordered">
                ...
                <tbody>
                <?php foreach ($iyzico_transactions as $transaction) { ?>
                <tr>
...
<?php if ($payment_success == 'yes') { ?>
<?php   if ($same_date == 'yes' && $successful_cancel_counts < 1 && $successful_refund_counts == 0) { ?>
    $("#btn_cancel").click(function () {
        if (confirm('<?php echo $text_are_you_sure; ?>')) {
            $.ajax({
                ...
                url: 'index.php?route=payment/iyzico_checkout_installment/cancel&token=<?php echo $token; ?>',
                beforeSend: function () {
                    $('#btn_cancel').hide();
    ...
    $("a.btn_refund").click(function(){
        ...
        if (confirm('<?php echo $text_are_you_sure; ?>')) {
            $.ajax({
                ...
                url: 'index.php?route=payment/iyzico_checkout_installment/refund&token=<?php echo $token; ?>',
                beforeSend: function () {
