...
class ControllerPaymentLaybuy extends Controller
{
    ...
    public function index()
    {
        ...
        $data['action'] = $this->url->link('payment/laybuy', 'token=' . $this->session->data['token'], 'SSL');

        $data['fetch'] = $this->url->link('payment/laybuy/fetch', 'token=' . $this->session->data['token'] . '#reportstab', 'SSL');

        $data['cancel'] = $this->url->link('extension/extension', 'filter_type=payment&token=' . $this->session->data['token'], 'SSL');
        ...
        $this->response->setOutput($this->load->view('payment/laybuy.tpl', $data));
    }
    ...
    public function transaction($order_page = false)
    {
        ...
        $data['cancel'] = $this->url->link('payment/laybuy', 'token=' . $this->session->data['token'] . '#reportstab', 'SSL');
        ...
        $this->response->setOutput($this->load->view('payment/laybuy_transaction.tpl', $data));
    }
    ...
    public function cancel()
    {
        ...
        if ($this->user->hasPermission('modify', 'payment/laybuy')) {
            ...
            if ($this->request->get['source'] == 'order') {
                $json['reload'] = $this->url->link('sale/order/info', 'order_id=' . (int) $transaction_info['order_id'] . '&token=' . $this->session->data['token'], 'SSL');
            } else {
                $json['reload'] = $this->url->link('payment/laybuy/transaction', 'token=' . $this->session->data['token'] . '&id=' . $id, 'SSL');
            }
    ...
    public function order()
    {
        if ($this->config->get('laybuy_status')) {
            ...
            return $this->load->view('payment/laybuy_order.tpl', $data);
        }
    }
